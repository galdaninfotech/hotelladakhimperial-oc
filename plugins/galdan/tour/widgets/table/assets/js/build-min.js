...
var Tour=function(tableObj){Base.call(this,tableObj)
...
this.data=JSON.parse(dataString)};Tour.prototype=Object.create(BaseProto)
Tour.prototype.constructor=Tour
Tour.prototype.dispose=function(){BaseProto.dispose.call(this)
this.data=null}
Tour.prototype.getRecords=function(offset,count,onSuccess){if(!count){onSuccess(this.data,this.data.length)}
else{onSuccess(this.data.slice(offset,offset+count),this.data.length)}}
Tour.prototype.createRecord=function(recordData,placement,relativeToKey,offset,count,onSuccess){if(placement==='bottom'){this.data.push(recordData)}
...
Tour.prototype.updateRecord=function(key,recordData){var recordIndex=this.getIndexOfKey(key)
...
Tour.prototype.deleteRecord=function(key,newRecordData,offset,count,onSuccess){var recordIndex=this.getIndexOfKey(key)
if(recordIndex!==-1){this.data.splice(recordIndex,1)
if(this.data.length==0)
this.data.push(newRecordData)
this.getRecords(offset,count,onSuccess)}
else{throw new Error('Record with they key '+key+' is not found in the data set')}}
Tour.prototype.getIndexOfKey=function(key){var keyColumn=this.tableObj.options.keyColumn
return this.data.map(function(record){return record[keyColumn]+""}).indexOf(key+"")}
Tour.prototype.getAllData=function(){return this.data}
$.oc.table.datasource.client=Tour}(window.jQuery);+function($){"use strict";if($.oc.table===undefined)
throw new Error("The $.oc.table namespace is not defined. Make sure that the table.js script is loaded.");if($.oc.table.datasource===undefined)
throw new Error("The $.oc.table.datasource namespace is not defined. Make sure that the table.datasource.base.js script is loaded.");var Base=$.oc.table.datasource.base,BaseProto=Base.prototype
